...
def run_comparative_analysis_threshold_permutation_imp_rf(sets_id, use_threshold=True, min_importance_feature_ef=0.05, min_mse_threshold = 0.00001):
    """
    STGP için Ridge + Permutation Importance (MSE Katkısı) yöntemini kullanır.
    EF için MDI (Mean Decrease Impurity) yöntemini kullanır.
    """
        ...
        try:
            ...
            if use_threshold:
                ...
                if not best_programs:
                    ...
                    # Ridge Modelini Eğit (Hakem)
                    judge_model = RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)
                    judge_model.fit(X_train_candidates, y_train)

                    # Permütasyon Önemini Hesapla (MSE Katkısı)
                    print("Training Judge Model (RandomForest) for Feature Selection...")
                    perm_result = permutation_importance(
                        judge_model,
                        X_train_candidates,
                        y_train,
            ...
            print(f'STGP(RandomForest-Selected)+EF {d_name} columns: {new_train_stgp_ef.shape[1]}')
...
def run_comparative_analysis_threshold_permutation_imp_xgb(sets_id, use_threshold=True, min_importance_feature_ef=0.05, min_mse_threshold = 0.00001):
    """
    STGP için Ridge + Permutation Importance (MSE Katkısı) yöntemini kullanır.
    EF için MDI (Mean Decrease Impurity) yöntemini kullanır.
    """
        ...
        try:
            ...
            if use_threshold:
                ...
                if not best_programs:
                    ...
                    # Ridge Modelini Eğit (Hakem)
                    judge_model = XGBRegressor(n_jobs=1, n_estimators=200, verbosity=0, random_state=42)
                    judge_model.fit(X_train_candidates, y_train)

                    # Permütasyon Önemini Hesapla (MSE Katkısı)
                    print("Training Judge Model (XGBoost) for Feature Selection...")
                    perm_result = permutation_importance(
                        judge_model,
                        X_train_candidates,
                        y_train,
            ...
            print(f'STGP(XGBoost-Selected)+EF {d_name} columns: {new_train_stgp_ef.shape[1]}')
...
def run_comparative_analysis_threshold_permutation_imp_all_algorithims(sets_id, use_threshold=True, min_importance_feature_ef=0.05, min_mse_threshold = 0.00001):
    """
    STGP için Ridge + Permutation Importance (MSE Katkısı) yöntemini kullanır.
    EF için MDI (Mean Decrease Impurity) yöntemini kullanır.
    """
    ...
    for set_id_val in sets_id:
        try:
            ...
            # --- Base Model Evaluation ---
            all_results.extend(evaluate_models_all_algorithims(X_train, X_test, y_train, y_test, d_name, "Base"))
            ...
            new_test_ef = np.hstack((X_test, X_test_ef))
            print(f'EF {d_name} columns: {new_train_ef.shape[1]}')
            all_results.extend(evaluate_models_all_algorithims(new_train_ef, new_test_ef, y_train, y_test, d_name, "EF"))
            ...
            all_results.extend(evaluate_models_all_algorithims(new_train_stgp_ef, new_test_stgp_ef,
                                               y_train, y_test, d_name, "STGP+EF"))
